...
def move(request):
    ...
    # pos_x = room.pos_x
    # pos_y = room.pos_y
    player_room_id = player.currentRoom
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        return JsonResponse({'name':player.user.username, 'player_room_id':player.currentRoom, 'title':nextRoom.title, 'description':nextRoom.description, 'players':players, 'error_msg':""}, safe=True)
    else:
        players = room.playerNames(player_id)
        return JsonResponse({'name':player.user.username, 'player_room_id':player.currentRoom, 'title':room.title, 'description':room.description, 'players':players, 'error_msg':"You cannot move that way."}, safe=True)
